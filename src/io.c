...
CURLcode urlFetchData(URL_t *URL, unsigned long bufSize) {
  ...
  if (URL->filePos != (size_t)-1)
    URL->filePos += URL->bufLen;
  else
    URL->filePos = 0;
...
URL_t *urlOpen(char *fname, CURLcode (*callBack)(CURL *), const char *mode) {
  ...
  if ((!mode) || (strchr(mode, 'w') == 0)) {
    ...
    if (strncmp(fname, "http://", 7) == 0)
      URL->type = BWG_HTTP;
    else if (strncmp(fname, "https://", 8) == 0)
      URL->type = BWG_HTTPS;
    else if (strncmp(fname, "ftp://", 6) == 0)
      URL->type = BWG_FTP;
    else
      URL->type = BWG_FILE;
#else
    ...
    if (URL->type == BWG_FILE) {
      ...
      if (curl_easy_setopt(URL->x.curl, CURLOPT_HTTPAUTH, CURLAUTH_ANY) != CURLE_OK) {
        fprintf(stderr,
                "[urlOpen] Failed instructing curl to use any HTTP authentication it finds to be "
                "suitable!\n");
        goto error;
      }
      // Follow redirects
      ...
      // Set the callback info, which means we no longer need to directly deal with sockets and
      // header!
