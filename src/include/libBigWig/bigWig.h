 ...
 *
 * libBigWig is a C library for parsing local/remote bigWig and bigBed files. This is similar to
 * Kent's library from UCSC, except
 ...
 *
 * There are three methods for storing values in a bigWig file, further described in the [wiggle
 * format](http://genome.ucsc.edu/goldenpath/help/wiggle.html). The entries within the file are
 * grouped into "blocks" and each such block is limited to storing entries of a single type. So, it
 * is unwise to use a single bedGraph-like endtry followed by a single fixed-step entry followed by
 * a variable-step entry, as that would require three separate blocks, with additional space
 * required for each.
 *
 * \section Testing file types
 *
 * As of version 0.3.0, libBigWig supports reading bigBed files. If an application needs to support
 * both bigBed and bigWig input, then the `bwIsBigWig` and `bbIsBigBed` functions can be used to
 * determine the file type. These both use the "magic" number at the beginning of the file to
 * determine the file type.
 *
 * \section Interval and entry iterators
 *
 * As of version 0.3.0, libBigWig supports iterating over intervals in bigWig files and entries in
 * bigBed files. The number of intervals/entries returned with each iteration can be controlled by
 * setting the number of blocks processed in each iteration (intervals and entries are group inside
 * of bigWig and bigBed files into blocks of entries). See `test/testIterator.c` for an example.
 *
 * \section Examples
 *
...
/*! \file bigWig.h
 *
 * These are the functions and structured that should be used by external users. While I don't
 * particularly recommend dealing with some of the structures (e.g., a bigWigHdr_t), they're
 * described here in case you need them.
 *
 * BTW, this library doesn't switch endianness as appropriate, since I kind of assume that there's
 * only one type produced these days.
 */
...
/*!
 * @brief BigWig files have multiple "zoom" levels, each of which has its own header. This hold
 * those headers
 *
...
typedef struct {
  ...
  uint64_t
      *dataOffset; /**<The offset to the on-disk start of the data. This isn't used currently.*/
...
typedef struct {
  ...
  uint64_t nEntries;  /**<The number of entries processed. This is used for the first contig and
                         determining how the zoom levels are computed*/
  uint64_t runningWidthSum; /**<The running sum of the entry widths for the first contig (again,
                               used for the first contig and computing zoom levels)*/
  ...
  uint64_t *
      nNodes; /**<The number of leaf nodes per zoom level, useful for determining duplicate levels*/
 ...
 */
typedef struct {
  URL_t *URL;       /**<A pointer that can handle both local and remote files (including a buffer if
                       needed).*/
 ...
 * One of intervals and entries should be used to access records from bigWig or bigBed files,
 * respectively.
 */
typedef struct {
  ...
  void *data; /**<Points to either intervals or entries. If there are no further intervals/entries,
                 then this is NULL. Use this to test for whether to continue iterating.*/
} bwOverlapIterator_t;

/*!
 * @brief Initializes curl and global variables. This *MUST* be called before other functions (at
 * least if you want to connect to remote files). For remote file, curl must be initialized and
 * regions of a file read into an internal buffer. If the buffer is too small then an excessive
 * number of connections will be made. If the buffer is too large than more data than required is
 * fetched. 128KiB is likely sufficient for most needs.
 ...
 * This function will quickly check either local or remote files to determine if they appear to be
 * valid bigWig files. This can be determined by reading the first 4 bytes of the file.
 * @param fname The file name or URL (http, https, and ftp are supported)
 * @param callBack An optional user-supplied function. This is applied to remote connections so
 * users can specify things like proxy and password information. See `test/testRemote` for an
 * example.
 ...
 * users can specify things like proxy and password information. See `test/testRemote` for an
 * example.
 * @param mode The mode, by default "r". Both local and remote files can be read, but only local
 * files can be written. For files being written the callback function is ignored. If and only if
 * the mode contains "w" will the file be opened for writing (in all other cases the file will be
 * opened for reading.
 ...
 * This will open a local or remote bigBed file. Note that this file format can only be read and NOT
 * written!
 * @param fname The file name or URL (http, https, and ftp are supported)
 * @param callBack An optional user-supplied function. This is applied to remote connections so
 * users can specify things like proxy and password information. See `test/testRemote` for an
 ...
 * @return An ID, -1 will be returned on error (note that this is an unsigned value, so that's ~4
 * billion. bigWig/bigBed files can't store that many chromosomes anyway.
 */
 ...
 * @param start The start position of the interval. This is 0-based half open, so 0 is the first
 * base.
 * @param end The end position of the interval. Again, this is 0-based half open, so 100 will
 * include the 100th base...which is at position 99.
 * @return NULL on error or no overlapping values, otherwise a `bwOverlappingIntervals_t *` holding
 * the values and intervals.
 * @see bwOverlappingIntervals_t
 * @see bwDestroyOverlappingIntervals
 * @see bwGetValues
 */
bwOverlappingIntervals_t *bwGetOverlappingIntervals(bigWigFile_t *fp, char *chrom, uint32_t start,
                                                    uint32_t end);
 ...
 * @param end The end position of the interval. Again, this is 0-based half open, so 100 will
 * include the 100th base...which is at position 99.
 * @param withString If not 0, return the string associated with each entry in the output. If 0,
 * there are no associated strings returned. This is useful if the only information needed are the
 * locations of the entries, which require significantly less memory.
 * @return NULL on error or no overlapping values, otherwise a `bbOverlappingEntries_t *` holding
 * the intervals and (optionally) the associated string.
 * @see bbOverlappingEntries_t
 * @see bbDestroyOverlappingEntries
 */
bbOverlappingEntries_t *bbGetOverlappingEntries(bigWigFile_t *fp, char *chrom, uint32_t start,
                                                uint32_t end, int withString);
 ...
 * Intervals are in the `intervals` member and `data` can be used to determine when to end
 * iteration.
 * @param fp A valid bigWigFile_t pointer. This MUST be for a bigWig file!
 * @param chrom A valid chromosome name.
 * @param start The start position of the interval. This is 0-based half open, so 0 is the first
 ...
 * @param end The end position of the interval. Again, this is 0-based half open, so 100 will
 * include the 100th base...which is at position 99.
 * @param blocksPerIteration The number of blocks (internal groupings of intervals in bigWig files)
 * to return per iteration.
 * @return NULL on error, otherwise a bwOverlapIterator_t pointer
 * @see bwOverlapIterator_t
 * @see bwIteratorNext
 * @see bwIteratorDestroy
 */
bwOverlapIterator_t *bwOverlappingIntervalsIterator(bigWigFile_t *fp, char *chrom, uint32_t start,
                                                    uint32_t end, uint32_t blocksPerIteration);
 ...
 * @param blocksPerIteration The number of blocks (internal groupings of entries in bigBed files) to
 ...
 * @see bwIteratorDestroy
 */
bwOverlapIterator_t *bbOverlappingEntriesIterator(bigWigFile_t *fp, char *chrom, uint32_t start,
                                                  uint32_t end, int withString,
 ...
 * @return NULL on error, otherwise a bwOverlapIterator_t pointer with the intervals or entries from
 ...
 * Given an interval (e.g., chr1:0-100), return the value at each position in a bigWig file.
 * Positions without associated values are suppressed by default, but may be returned if `includeNA`
 * is not 0.
 * @param fp A valid bigWigFile_t pointer.
 * @param chrom A valid chromosome name.
 * @param start The start position of the interval. This is 0-based half open, so 0 is the first
 ...
 * @return NULL on error or no overlapping values, otherwise a `bwOverlappingIntervals_t *` holding
 * the values and positions.
 * @see bwOverlappingIntervals_t
 * @see bwDestroyOverlappingIntervals
 * @see bwGetOverlappingIntervals
 */
bwOverlappingIntervals_t *bwGetValues(bigWigFile_t *fp, char *chrom, uint32_t start, uint32_t end,
                                      int includeNA);

/*!
 * @brief Determines per-interval bigWig statistics
 * Can determine mean/min/max/coverage/standard deviation of values in one or more intervals in a
 * bigWig file. You can optionally give it an interval and ask for values from X number of
 * sub-intervals.
 ...
 * @see bwStatsType
 * @return A pointer to an array of double precission floating point values. Note that bigWig files
 * only hold 32-bit values, so this is done to help prevent overflows.
 */
double *bwStats(bigWigFile_t *fp, char *chrom, uint32_t start, uint32_t end, uint32_t nBins,
 ...
 * Can determine mean/min/max/coverage/standard deviation of values in one or more intervals in a
 * bigWig file. You can optionally give it an interval and ask for values from X number of
 * sub-intervals. The difference with bwStats is that zoom levels are never used.
 * @param fp The file from which to extract statistics.
 * @param chrom A valid chromosome name.
 * @param start The start position of the interval. This is 0-based half open, so 0 is the first
 * base.
 ...
 * only hold 32-bit values, so this is done to help prevent overflows.
 */
double *bwStatsFromFull(bigWigFile_t *fp, char *chrom, uint32_t start, uint32_t end, uint32_t nBins,
                        enum bwStatsType type);
 ...
 * Every bigWig file has a header, this creates the template for one. It also takes care of space
 * allocation in the output write buffer.
 * @param fp The bigWigFile_t* that you want to write to.
 * @param maxZooms The maximum number of zoom levels. If you specify 0 then there will be no zoom
 * levels. A value <0 or > 65535 will result in a maximum of 10.
 * @return 0 on success.
 */
 ...
 * to the file. These will always be added in a new block, so you may have previously used a
 * different storage type.
 *
 * In general it's more efficient to use the bwAppend* functions, but then you MUST know that the
 * previously written block is of the same type. In other words, you can only use
 * bwAppendIntervals() after bwAddIntervals() or a previous bwAppendIntervals().
 ...
 */
int bwAddIntervals(bigWigFile_t *fp, char **chrom, uint32_t *start, uint32_t *end, float *values,
                   uint32_t n);

/*!
 * @brief Append bedGraph-like intervals to a previous block of bedGraph-like intervals in a bigWig
 * file. If you have previously used bwAddIntervals() then this will append additional entries into
 * the previous block (or start a new one if needed).
 * @param fp The output file pointer.
 * @param start A list of start positions of length`n`.
 * @param end A list of end positions of length`n`.
 * @param values A list of values of length`n`.
 * @param n The length of the aforementioned lists.
 * @return 0 on success and another value on error.
 * @warning Do NOT use this after `bwAddIntervalSpanSteps()`, `bwAppendIntervalSpanSteps()`,
 * `bwAddIntervalSpanSteps()`, or `bwAppendIntervalSpanSteps()`.
 * @see bwAddIntervals
 */
 ...
 * to the file. Each block of such entries has an associated "span", so each value describes the
 * region chromosome:start-(start+span)
 *
 ...
 */
int bwAddIntervalSpans(bigWigFile_t *fp, char *chrom, uint32_t *start, uint32_t span, float *values,
                       uint32_t n);

/*!
 * @brief Append to a previous block of variable-step entries.
 * If you previously used `bwAddIntervalSpans()`, this will continue appending more values to the
 * block(s) it created.
 * @param fp The output file pointer.
 * @param start A list of start positions of length`n`.
 * @param values A list of values of length`n`.
 * @param n The length of the aforementioned lists.
 * @return 0 on success and another value on error.
 * @warning Do NOT use this after `bwAddIntervals()`, `bwAppendIntervals()`,
 * `bwAddIntervalSpanSteps()` or `bwAppendIntervalSpanSteps()`
 ...
 * Adds entries for the form
 * value
 * to the file. Each block of such entries has an associated "span", "step", chromosome and start
 * position. See the wiggle format for more details.
 *
 * This will always start a new block of values.
 * @param fp The output file pointer.
 ...
 */
int bwAddIntervalSpanSteps(bigWigFile_t *fp, char *chrom, uint32_t start, uint32_t span,
                           uint32_t step, float *values, uint32_t n);

/*!
 * @brief Append to a previous block of fixed-step entries.
 * If you previously used `bwAddIntervalSpanSteps()`, this will continue appending more values to
 * the block(s) it created.
 * @param fp The output file pointer.
 * @param values A list of values of length`n`.
 * @param n The length of the aforementioned lists.
 * @return 0 on success and another value on error.
 * @warning Do NOT use this after `bwAddIntervals()`, `bwAppendIntervals()`, `bwAddIntervalSpans()`
