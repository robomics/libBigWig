...
/*! \file bwValues.h
 *
 * You should not directly use functions and structures defined here. They're really meant for
 * internal use only.
 *
 * All of the structures here need to be destroyed or you'll leak memory! There are methods
 * available to destroy anything that you need to take care of yourself.
 */
 ...
 *
 * Note that there are two types of nodes: leaf and twig. Leaf nodes point to where data actually
 * is. Twig nodes point to additional index nodes, which may or may not be leaves. Each of these
 * nodes has additional children, which may span multiple chromosomes/contigs.
 *
 * With the start/end position, these positions refer specifically to the chromosomes specified in
 * chrIdxStart/chrIdxEnd. Any chromosomes between these are completely spanned by a given child.
 */
typedef struct bwRTreeNode_t {
  ...
  uint64_t *dataOffset; /**<For leaves, the offset to the on-disk data. For twigs, the offset to the
                           child node.*/
  union {
...
typedef struct {
  ...
  uint64_t nItems;    /**<The total number of data blocks pointed to by the tree. This is completely
                         redundant.*/
  ...
  uint64_t idxSize; /**<This is actually the offset of the index rather than the size?!? Yes, it's
                       completely redundant.*/
  uint32_t nItemsPerSlot; /**<This is always 1!*/
  // There's 4 bytes of padding in the file here
  uint64_t
      rootOffset; /**<The offset to the root node of the R-Tree (on disk). Yes, this is redundant.*/
 ...
 *
 * There are 3 types of data blocks in bigWig files, each with slightly different needs. This is all
