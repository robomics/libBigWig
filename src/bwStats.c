...
// Returns -1 if there are no applicable levels, otherwise an integer indicating the most
// appropriate level. Like Kent's library, this divides the desired bin size by 2 to minimize the
// effect of blocks overlapping multiple bins
...
double getScalar(uint32_t i_start, uint32_t i_end, uint32_t b_start, uint32_t b_end) {
  ...
  if (b_start <= i_start) {
    ...
    if (b_end < i_end)
      rv = ((double)(b_end - b_start)) / (b_end - b_start);
    else
      rv = ((double)(i_end - b_start)) / (b_end - b_start);
  }
...
static double blockMean(bigWigFile_t *fp, bwOverlapBlock_t *blocks, uint32_t tid, uint32_t start,
                        uint32_t end) {
  uint32_t i, j;
...
static double blockDev(bigWigFile_t *fp, bwOverlapBlock_t *blocks, uint32_t tid, uint32_t start,
                       uint32_t end) {
  uint32_t i, j;
...
static double intDev(bwOverlappingIntervals_t *ints, uint32_t start, uint32_t end) {
  ...
  for (i = 0; i < ints->l; i++) {
    ...
    if (ints->end[i] > end) end_use = end;
    nBases += end_use - start_use;
    v1 += (end_use - start_use) *
          pow(ints->value[i] - mean, 2.0);  // running sum of squared difference
  }
...
static double blockMax(bigWigFile_t *fp, bwOverlapBlock_t *blocks, uint32_t tid, uint32_t start,
                       uint32_t end) {
  uint32_t i, j, isNA = 1;
...
static double blockMin(bigWigFile_t *fp, bwOverlapBlock_t *blocks, uint32_t tid, uint32_t start,
                       uint32_t end) {
  uint32_t i, j, isNA = 1;
  double o = strtod("NaN", NULL);
  struct vals_t *v = NULL;
...
static double blockCoverage(bigWigFile_t *fp, bwOverlapBlock_t *blocks, uint32_t tid,
                            uint32_t start, uint32_t end) {
  uint32_t i, j;
  double o = 0.0;
  struct vals_t *v = NULL;
...
static double blockSum(bigWigFile_t *fp, bwOverlapBlock_t *blocks, uint32_t tid, uint32_t start,
                       uint32_t end) {
...
double *bwStatsFromZoom(bigWigFile_t *fp, int32_t level, uint32_t tid, uint32_t start, uint32_t end,
                        uint32_t nBins, enum bwStatsType type) {
