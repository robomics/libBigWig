...
// If maxZooms == 0, then 0 is used (i.e., there are no zoom levels). If maxZooms < 0 or > 65535
// then 10 is used.
...
int bwCreateHdr(bigWigFile_t *fp, int32_t maxZooms) {
  ...
  hdr->bufSize =
      32768;  // When the file is finalized this is reset if fp->writeBuffer->compressPsz is 0!
  hdr->minVal = DBL_MAX;
  hdr->maxVal = DBL_MIN;
  fp->hdr = hdr;
...
static int writeChromList(FILE *fp, chromList_t *cl) {
  ...
  uint32_t nperblock =
      (cl->nKeys > 0x7FFF)
          ? 0x7FFF
          : cl->nKeys;  // Items per leaf/non-leaf, there are no unsigned ints in java :(
  uint32_t nblocks,
      keySize = 0,
      valSize = 8;  // In theory valSize could be optimized, in practice that'd be annoying
  ...
  if (cl->nKeys > 1073676289) {
    fprintf(stderr,
            "[writeChromList] Error: Currently only 1,073,676,289 contigs are supported. If you "
            "really need more then please post a request on github.\n");
    return 1;
  }
...
// 0 on success
static int appendIndexNodeEntry(bigWigFile_t *fp, uint32_t tid0, uint32_t tid1, uint32_t start,
                                uint32_t end, uint64_t offset, uint64_t size) {
...
// Returns 0 on success
static int addIndexEntry(bigWigFile_t *fp, uint32_t tid0, uint32_t tid1, uint32_t start,
                         uint32_t end, uint64_t offset, uint64_t size) {
  bwRTreeNode_t *node;
...
/*
 * TODO:
 *     The buffer size and compression sz need to be determined elsewhere (and p and compressP
 * filled in!)
 */
static int flushBuffer(bigWigFile_t *fp) {
  ...
  // Fill in the header
  uint8_t *p = wb->p;
  if (!memcpy(p, &(wb->tid), sizeof(uint32_t))) return 1;
  if (!memcpy(p + 4, &(wb->start), sizeof(uint32_t))) return 2;
  if (!memcpy(p + 8, &(wb->end), sizeof(uint32_t))) return 3;
  if (!memcpy(p + 12, &(wb->step), sizeof(uint32_t))) return 4;
  if (!memcpy(p + 16, &(wb->span), sizeof(uint32_t))) return 5;
  if (!memcpy(p + 20, &(wb->ltype), sizeof(uint8_t))) return 6;
  // 1 byte padding
  ...
  if (!memcpy(p + 22, &nItems, sizeof(uint16_t))) return 8;
...
static void updateStats(bigWigFile_t *fp, uint32_t span, float val) {
  if (val < fp->hdr->minVal)
    fp->hdr->minVal = val;
  else if (val > fp->hdr->maxVal)
    fp->hdr->maxVal = val;
...
int bwAddIntervals(bigWigFile_t *fp, char **chrom, uint32_t *start, uint32_t *end, float *values,
  ...
  if (!memcpy(p + wb->l, start, sizeof(uint32_t))) return 7;
  if (!memcpy(p + wb->l + 4, end, sizeof(uint32_t))) return 8;
  if (!memcpy(p + wb->l + 8, values, sizeof(float))) return 9;
  ...
  for (i = 1; i < n; i++) {
    ...
    if (!memcpy(p + wb->l, &(start[i]), sizeof(uint32_t))) return 11;
    if (!memcpy(p + wb->l + 4, &(end[i]), sizeof(uint32_t))) return 12;
    if (!memcpy(p + wb->l + 8, &(values[i]), sizeof(float))) return 13;
...
int bwAppendIntervals(bigWigFile_t *fp, uint32_t *start, uint32_t *end, float *values, uint32_t n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    uint8_t *p = wb->p;
    if (!memcpy(p + wb->l, &(start[i]), sizeof(uint32_t))) return 4;
    if (!memcpy(p + wb->l + 4, &(end[i]), sizeof(uint32_t))) return 5;
    if (!memcpy(p + wb->l + 8, &(values[i]), sizeof(float))) return 6;
    updateStats(fp, end[i] - start[i], values[i]);
    wb->l += 12;
  }
  wb->end = end[i - 1];
...
int bwAddIntervalSpans(bigWigFile_t *fp, char *chrom, uint32_t *start, uint32_t span, float *values,
  ...
  for (i = 0; i < n; i++) {
    ...
    uint8_t *p = wb->p;
    if (!memcpy(p + wb->l, &(start[i]), sizeof(uint32_t))) return 5;
    if (!memcpy(p + wb->l + 4, &(values[i]), sizeof(float))) return 6;
...
int bwAppendIntervalSpans(bigWigFile_t *fp, uint32_t *start, float *values, uint32_t n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    uint8_t *p = wb->p;
    if (!memcpy(p + wb->l, &(start[i]), sizeof(uint32_t))) return 4;
    if (!memcpy(p + wb->l + 4, &(values[i]), sizeof(float))) return 5;
...
int bwAddIntervalSpanSteps(bigWigFile_t *fp, char *chrom, uint32_t start, uint32_t span,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!memcpy((uint8_t *)wb->p + wb->l, &(values[i]), sizeof(float))) return 5;
    updateStats(fp, wb->span, values[i]);
    wb->l += 4;
  }
...
int bwAppendIntervalSpanSteps(bigWigFile_t *fp, float *values, uint32_t n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!memcpy((uint8_t *)wb->p + wb->l, &(values[i]), sizeof(float))) return 4;
    updateStats(fp, wb->span, values[i]);
    wb->l += 4;
  }
...
int writeSummary(bigWigFile_t *fp) {
  if (writeAtPos(&(fp->hdr->nBasesCovered), sizeof(uint64_t), 1, fp->hdr->summaryOffset,
                 fp->URL->x.fp))
    return 1;
  if (writeAtPos(&(fp->hdr->minVal), sizeof(double), 1, fp->hdr->summaryOffset + 8, fp->URL->x.fp))
    return 2;
  if (writeAtPos(&(fp->hdr->maxVal), sizeof(double), 1, fp->hdr->summaryOffset + 16, fp->URL->x.fp))
    return 3;
  if (writeAtPos(&(fp->hdr->sumData), sizeof(double), 1, fp->hdr->summaryOffset + 24,
                 fp->URL->x.fp))
    return 4;
  if (writeAtPos(&(fp->hdr->sumSquared), sizeof(double), 1, fp->hdr->summaryOffset + 32,
                 fp->URL->x.fp))
    return 5;
  return 0;
}
...
int writeIndex(bigWigFile_t *fp) {
  ...
  if (ll == fp->writeBuffer->currentIndexNode) {
    ...
    root = addLeaves(&ll, &idxSize,
                     ceil(((double)fp->writeBuffer->nBlocks) / fp->writeBuffer->blockSize),
                     fp->writeBuffer->blockSize);
  }
  if (!root) return 4;
  fp->idx->root = root;
  ...
  if (fwrite(&(root->chrIdxEnd[root->nChildren - 1]), sizeof(uint32_t), 1, fp->URL->x.fp) != 1)
    return 10;
  if (fwrite(&(root->baseEnd[root->nChildren - 1]), sizeof(uint32_t), 1, fp->URL->x.fp) != 1)
    return 11;
  ...
  for (i = 0; i < root->nChildren; i++) {
    ...
    if (root->isLeaf) {
      ...
      root->dataOffset[i] =
          0;  // FIXME: Something upstream is setting this to impossible values (e.g., 0x21?!?!?)
...
int makeZoomLevels(bigWigFile_t *fp) {
  ...
  for (i = 0; i < fp->hdr->nLevels; i++) {
    ...
    if (fp->hdr->zoomHdrs->level[i] > fp->cl->len[0])
      ((uint32_t *)fp->writeBuffer->firstZoomBuffer[i]->p)[2] = fp->cl->len[0];
...
uint32_t overlapsInterval(uint32_t tid0, uint32_t start0, uint32_t end0, uint32_t tid1,
...
uint32_t updateInterval(bigWigFile_t *fp, bwZoomBuffer_t *buffer, double *sum, double *sumsq,
                        uint32_t size, uint32_t tid, uint32_t start, uint32_t end, float value) {
  ...
  if (offset) {
    rv = overlapsInterval(p2[8 * (offset - 1)], p2[8 * (offset - 1) + 1],
                          p2[8 * (offset - 1) + 1] + size, tid, start, end);
    if (rv) {
  ...
  // If we move to a new interval then skip iterating over a bunch of obviously non-overlapping
  // intervals
  if (offset && p2[8 * offset + 2] == 0) {
    p2[8 * offset] = tid;
    p2[8 * offset + 1] = start;
    if (start + size < end)
      p2[8 * offset + 2] = start + size;
    else
      p2[8 * offset + 2] = end;
  ...
  // Add a new entry
  while (!(rv = overlapsInterval(p2[8 * offset], p2[8 * offset + 1], p2[8 * offset + 1] + size, tid,
                                 start, end))) {
    p2[8 * offset] = tid;
    p2[8 * offset + 1] = start;
    if (start + size < end)
...
// Returns 0 on success
int addIntervalValue(bigWigFile_t *fp, uint64_t *nEntries, double *sum, double *sumsq,
                     bwZoomBuffer_t *buffer, uint32_t itemsPerSlot, uint32_t zoom, uint32_t tid,
  ...
  while (start < end) {
    ...
    if (!rv) {
      ...
      memcpy(newBuffer->p, (uint8_t *)buffer->p + buffer->l - 32, 4);
      memcpy((uint8_t *)newBuffer->p + 4, (uint8_t *)buffer->p + buffer->l - 28, 4);
    ...
    while (it->data != NULL) {
      for (j = 0; j < it->intervals->l; j++) {
        for (k = 0; k < fp->hdr->nLevels; k++) {
          if (addIntervalValue(fp, &(fp->writeBuffer->nNodes[k]), sum + k, sumsq + k,
                               fp->writeBuffer->lastZoomBuffer[k], fp->hdr->bufSize / 32,
                               fp->hdr->zoomHdrs->level[k], i, it->intervals->start[j],
                               it->intervals->end[j], it->intervals->value[j]))
            goto error;
          while (fp->writeBuffer->lastZoomBuffer[k]->next)
            fp->writeBuffer->lastZoomBuffer[k] = fp->writeBuffer->lastZoomBuffer[k]->next;
        }
      }
...
int writeZoomLevels(bigWigFile_t *fp) {
  ...
  for (i = 0; i < fp->hdr->nLevels; i++) {
    ...
    while (zb) {
      ...
      // Add an entry into the index
      last = (zb->l - 32) >> 2;
      if (addIndexEntry(fp, ((uint32_t *)zb->p)[0], ((uint32_t *)zb->p)[last],
                        ((uint32_t *)zb->p)[1], ((uint32_t *)zb->p)[last + 2], bwTell(fp) - sz, sz))
        return 4;
    ...
    if (writeAtPos(&(wb->nBlocks), sizeof(uint32_t), 1, fp->hdr->zoomHdrs->dataOffset[i],
                   fp->URL->x.fp))
      return 5;
  ...
  for (i = 0; i < actualNLevels; i++) {
    ...
    if (fwrite(&(fp->hdr->zoomHdrs->dataOffset[i]), sizeof(uint64_t), 1, fp->URL->x.fp) != 1)
      return 10;
    if (fwrite(&(fp->hdr->zoomHdrs->indexOffset[i]), sizeof(uint64_t), 1, fp->URL->x.fp) != 1)
      return 11;
  }
...
int bwFinalize(bigWigFile_t *fp) {
  ...
  if (fp->hdr) {
    if (writeAtPos(&(fp->writeBuffer->nBlocks), sizeof(uint64_t), 1, fp->hdr->dataOffset,
                   fp->URL->x.fp))
      return 2;
  } else {
