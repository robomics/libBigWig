...
error:
  ...
  if (node->isLeaf && node->x.size)
    free(node->x.size);
  else if ((!node->isLeaf) && node->x.child)
    free(node->x.child);
  free(node);
  return NULL;
}
...
static bwOverlapBlock_t *overlapsLeaf(bwRTreeNode_t *node, uint32_t tid, uint32_t start,
                                      uint32_t end) {
  uint16_t i, idx = 0;
...
// Returns NULL and sets nOverlaps to >0 on error, otherwise nOverlaps is the number of file offsets
// returned The output needs to be free()d if not NULL (likewise with *sizes)
static bwOverlapBlock_t *overlapsNonLeaf(bigWigFile_t *fp, bwRTreeNode_t *node, uint32_t tid,
  ...
  for (i = 0; i < node->nChildren; i++) {
    ...
    // We have an overlap!
    if (!node->x.child[i]) node->x.child[i] = bwGetRTreeNode(fp, node->dataOffset[i]);
...
// Returns NULL and sets nOverlaps to >0 on error, otherwise nOverlaps is the number of file offsets
// returned The output must be free()d
bwOverlapBlock_t *walkRTreeNodes(bigWigFile_t *bw, bwRTreeNode_t *root, uint32_t tid,
                                 uint32_t start, uint32_t end) {
...
static bwOverlapBlock_t *bwGetOverlappingBlocks(bigWigFile_t *fp, char *chrom, uint32_t start,
                                                uint32_t end) {
...
static bwOverlappingIntervals_t *pushIntervals(bwOverlappingIntervals_t *o, uint32_t start,
                                               uint32_t end, float value) {
  if (o->l + 1 >= o->m) {
    o->m = roundup(o->l + 1);
...
static bbOverlappingEntries_t *pushBBIntervals(bbOverlappingEntries_t *o, uint32_t start,
                                               uint32_t end, char *str, int withString) {
  if (o->l + 1 >= o->m) {
    o->m = roundup(o->l + 1);
    o->start = realloc(o->start, o->m * sizeof(uint32_t));
    if (!o->start) goto error;
...
bwOverlappingIntervals_t *bwGetOverlappingIntervalsCore(bigWigFile_t *fp, bwOverlapBlock_t *o,
                                                        uint32_t tid, uint32_t ostart,
                                                        uint32_t oend) {
  uint64_t i;
  uint16_t j;
  int compressed = 0, rv;
...
bbOverlappingEntries_t *bbGetOverlappingEntriesCore(bigWigFile_t *fp, bwOverlapBlock_t *o,
                                                    uint32_t tid, uint32_t ostart, uint32_t oend,
                                                    int withString) {
  uint64_t i;
...
// Returns NULL on error
bwOverlapIterator_t *bwOverlappingIntervalsIterator(bigWigFile_t *bw, char *chrom, uint32_t start,
...
// Returns NULL on error
bwOverlapIterator_t *bbOverlappingEntriesIterator(bigWigFile_t *bw, char *chrom, uint32_t start,
                                                  uint32_t end, int withString,
...
bwOverlapIterator_t *bwIteratorNext(bwOverlapIterator_t *iter) {
  ...
  if (iter->offset < blocks->n) {
    ...
    if (iter->bw->type == 0) {
      // bigWig
      iter->intervals =
          bwGetOverlappingIntervalsCore(iter->bw, blocks, iter->tid, iter->start, iter->end);
      iter->data = iter->intervals;
    } else {
      // bigBed
      iter->entries = bbGetOverlappingEntriesCore(iter->bw, blocks, iter->tid, iter->start,
                                                  iter->end, iter->withString);
...
// This is like bwGetOverlappingIntervals, except it returns 1 base windows. If includeNA is not 0,
// then a value will be returned for every position in the range (defaulting to NAN). The ->end
// member is NULL If includeNA is not 0 then ->start is also NULL, since it's implied Note that
// bwDestroyOverlappingIntervals() will work in either case
...
// Returns a pointer to the requested index (@offset, unless it's 0, in which case the index for the
// values is returned Returns NULL on error
