...
static void bwHdrRead(bigWigFile_t *bw) {
  ...
  if (bwRead((void *)&(bw->hdr->definedFieldCount), sizeof(uint16_t), 1, bw) != 1)
    goto error;                                                                             // 0x22
  ...
  if (bwRead((void *)&(bw->hdr->extensionOffset), sizeof(uint64_t), 1, bw) != 1)
    goto error;  // 0x38
...
void bwClose(bigWigFile_t *fp) {
  ...
  if (bwFinalize(fp)) {
    fprintf(stderr,
            "[bwClose] There was an error while finishing writing a bigWig file! The output is "
            "likely truncated.\n");
  }
