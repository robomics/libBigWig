...
void bwPrintHdr(bigWigFile_t *bw) {
  ...
  for (i = 0; i < bw->hdr->nLevels; i++) {
    printf("\t%" PRIu64 "\t%" PRIu32 "\t%" PRIx64 "\t%" PRIx64 "\n", i, bw->hdr->zoomHdrs->level[i],
           bw->hdr->zoomHdrs->dataOffset[i], bw->hdr->zoomHdrs->indexOffset[i]);
  }
  ...
  for (i = 0; i < node->nChildren; i++) {
    if (node->isLeaf) {
      printf("  %i\t%" PRIu32 "\t%" PRIu32 "\t%" PRIu32 "\t%" PRIu32 "\t0x%" PRIx64 "\t%" PRIu64
             "\n",
             level, node->chrIdxStart[i], node->baseStart[i], node->chrIdxEnd[i], node->baseEnd[i],
             node->dataOffset[i], node->x.size[i]);
    } else {
      printf("  %i\t%" PRIu32 "\t%" PRIu32 "\t%" PRIu32 "\t%" PRIu32 "\t0x%" PRIx64 "\tNA\n", level,
             node->chrIdxStart[i], node->baseStart[i], node->chrIdxEnd[i], node->baseEnd[i],
             node->dataOffset[i]);
...
void printIntervals(bwOverlappingIntervals_t *ints, uint32_t start) {
  ...
  for (i = 0; i < ints->l; i++) {
    if (ints->start && ints->end) {
      printf("Interval %" PRIu32 "\t%" PRIu32 "-%" PRIu32 ": %f\n", i, ints->start[i], ints->end[i],
             ints->value[i]);
    } else if (ints->start) {
      printf("Interval %" PRIu32 "\t%" PRIu32 "-%" PRIu32 ": %f\n", i, ints->start[i],
             ints->start[i] + 1, ints->value[i]);
    } else {
      printf("Interval %" PRIu32 "\t%" PRIu32 "-%" PRIu32 ": %f\n", i, start + i, start + i + 1,
             ints->value[i]);
    }
