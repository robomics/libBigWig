/*! \file bwCommon.h
 *
 * You have no reason to use these functions. They may change without warning because there's no
 * reason for them to be used outside of libBigWig's internals.
 *
 * These are structures and functions from a variety of files that are used across files internally
 * but don't need to be see by libBigWig users.
 */

/*!
 * @brief Like fsetpos, but for local or remote bigWig files.
 * This will set the file position indicator to the specified point. For local files this literally
 * is `fsetpos`, while for remote files it fills a memory buffer with data starting at the desired
 * position.
 ...
 * @see bwSetPos
 * @return For nmemb==1, the size of the copied data. For nmemb>1, the number of members fully
 * copied (this is equivalent to `fread`).
 */
 ...
 * There is little reason for end users to use this function. This must be freed with
 * `bwDestroyIndex`
 ...
 * There is generally little reason to use this, since end users should typically not need to run
 * `bwReadIndex` themselves.
 ...
 * Flushes the buffer, converts the index linked list to a tree, writes that to disk, handles zoom
 * level stuff, writes magic at the end
 * @param fp A valid bigWigFile_t pointer
 * @return 0 on success
 */
